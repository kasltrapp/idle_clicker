...
**Top bar:** currency displays (Followers/Cash/Aura) each followed by a small "+" button (present and styled, functionally unwired for now — real Shop-screen wiring is a separate future task). Player avatar and hamburger menu are still not built — deferred, not yet reached in the build order.
