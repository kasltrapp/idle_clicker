...
{
    [AddComponentMenu("EasyIdleGame/Displayers/Business Displayer")]
    public class BusinessDisplayer : BaseDisplayer, IMetadataDisplayer
    {
        // IMetadataDisplayer
        ...
        {
            BigNumber buyAmount_ = buyAmount == 0 ? 1 : buyAmount;

            string businessNameString = showIconStringInBuyButton ? metadata?.GetIconString() : string.Empty;
            string priceString = showPriceInBuyButton ? $"({InputsToString(iBuyable.GetRealCostToInputs(buyAmount_))})" : string.Empty;
            string buyAmountString = showBuyAmount ? $"{buyAmount_}x" : string.Empty;
        ...
        public override void Redraw()
        {
            ...
            (this as IMetadataDisplayer).Redraw_Metadata(activeMetadata);
